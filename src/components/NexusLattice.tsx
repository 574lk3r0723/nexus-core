...
function LatticeNode({ position, color, pulse }: any) {
  ...
  useFrame((state) => {
    if (pulse && ref.current) {
      ...
      ref.current.scale.x = ref.current.scale.y = ref.current.scale.z = 1 + Math.sin(dist * 0.1 - state.clock.elapsedTime * 10) * 0.5;
    }
  });
...
export default function NexusLattice({ nodes, pulse }: any) {
  ...
  const colors = nodes.flatMap((n: any) => [n.color]);

  return (
    <Canvas camera={{ position: [0, 0, 600], fov: 60 }}>
      <ambientLight intensity={0.3} />
      <pointLight position={[0, 0, 500]} intensity={2} color="#ffffff" />
      <Points positions={new Float32Array(points)} colors={colors} stride={3}>
        {nodes.map((node: any) => (
