...
export default function NexusCore() {
    ...
    while (true) {
      ...
      for (const line of lines) {
        try {
          ...
          if (data.content) {
            setGlyphs(prev => prev + data.content);
          }
        } catch {}
      }
    }
  ...
  return (
    <div className="nexus-core w-screen h-screen relative overflow-hidden">
      ...
      <div className="fixed w-4 h-4 bg-white/50 rounded-full pointer-events-none mix-blend-screen animate-pulse-core -translate-x-1/2 -translate-y-1/2 z-50" />
    </div>
  );
}
